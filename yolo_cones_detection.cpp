...
*/
        ...
        for (auto l_det : result_left[0])
        {
            ...
            /*
            if(count_img>0){
                cv::imwrite("/home/raceupdv/Desktop/images_test/roi_"+std::to_string(count_img)+".jpg", roi);
                count_img--;
            }*/
                ...
                */
                ...
                if (abs(rbb_tl.y - lbb_tl.y) <= EPIPOLAR_TOLERANCE && l_det.score > SCORE_THRESHOLD && r_det.class_idx == l_det.class_idx && l_det.bbox.height > 10)// 
                {
                    ...
                    if (abs(tp_left.x - tp_right.x) < DISP_THRESHOLD) //&& 
                    {
                        ...
                        tops_l.push_back(tp_left);
                        tops_r.push_back(tp_right);

                        // float disparity = lbb_tl.x - rbb_tl.x;
                        float disparity = tp_left.x - tp_right.x;
                        if (disparity == 0)
                            disparity = 0.01;
                        ...
                        if(l_det.class_idx == 0)
                            sparse_pts_to_reproj_blue.push_back(cv::Point3f(tp_left.x, tp_left.y, disparity));
                        if(l_det.class_idx == 1)
                            sparse_pts_to_reproj_yellow.push_back(cv::Point3f(tp_left.x, tp_left.y, disparity));
                        if(l_det.class_idx == 3)
                            sparse_pts_to_reproj_big_orange.push_back(cv::Point3f(tp_left.x, tp_left.y, disparity));
                        else
                            sparse_pts_to_reproj_blue.push_back(cv::Point3f(tp_left.x, tp_left.y, disparity));
                        // start from right centers
                        sparse_pts_to_reproj_test.push_back(cv::Point3f(tp_right.x, tp_right.y, -disparity));
                        counter_match++;
                    }
                }
            }
