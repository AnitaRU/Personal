...
/*cv::Point segmentTopPoint(cv::Mat roi, int x, int y) {
    ...
    void img_callback(const bumblebee2_ros_driver::msg::StereoImage::SharedPtr img_msg){
        ...
        std::vector<cv::Point> tops_l, tops_r;
        ...
        //std::vector<cv::Point_<int>> topPointListLeft = segmentTopPoints(left_img_rgb, result_left[0]);
        //std::vector<cv::Point_<int>> topPointListRight = segmentTopPoints(right_img_rgb, result_right[0]);
      ...
      /*  for (auto r_det : result_right[0])
        {
            auto bb_tl = r_det.bbox.tl(); // retrieve tl corner of bb

            auto bb_br = bb_tl;         //calculate br corner of bb
            bb_br.x += r_det.bbox.width; 
            bb_br.y += r_det.bbox.height;

            // Extract the region of interest (ROI) corresponding to the bounding box
            roi = right_img_rgb(cv::Rect(bb_tl, bb_br));
            if (roi.channels() == 3) {
...
*/
        ...
        for (auto l_det : result_left[0])
        {
            std::cout<<"....................."<<counter_debug<<std::endl;
            auto lbb_tl = l_det.bbox.tl(); // retrieve tl corner of bb

            auto bb_br_left = lbb_tl;         //calculate br corner of bb
            bb_br_left.x += l_det.bbox.width; 
            ...
            for (auto r_det : result_right[0])
            {
                ...
                /*
                    ...
                    *      --> 1 / disparity
                */
                auto rbb_tl = r_det.bbox.tl(); // retrieve tl corner of bb

                auto bb_br_right = rbb_tl;         //calculate br corner of bb
                bb_br_right.x += r_det.bbox.width; 
                ...
                if (abs(rbb_tl.y - lbb_tl.y) <= EPIPOLAR_TOLERANCE && l_det.score > SCORE_THRESHOLD && r_det.class_idx == l_det.class_idx && l_det.bbox.height > 10)// 
                {
                    ...
                    //
                    cv::Point l_top(lbb_tl.x + (l_det.bbox.width / 2.0), lbb_tl.y + l_det.bbox.height / 2.0);
                    cv::Point r_top(rbb_tl.x + (r_det.bbox.width / 2.0), rbb_tl.y + r_det.bbox.height / 2.0);


                    // if (abs(lbb_tl.x - rbb_tl.x) < DISP_THRESHOLD)
                    if (abs(l_top.x - r_top.x) < DISP_THRESHOLD) //&& 
                    {
                        //centers_l.push_back(l_center);
                        //centers_r.push_back(r_center);
                        tops_l.push_back(l_top);
                        tops_r.push_back(r_top);

                        // float disparity = lbb_tl.x - rbb_tl.x;
                        float disparity = l_top.x - r_top.x;
                        if (disparity == 0)
                            disparity = 0.01;
                        ...
                        if(l_det.class_idx == 0)
                            sparse_pts_to_reproj_blue.push_back(cv::Point3f(l_top.x, l_top.y, disparity));
                        if(l_det.class_idx == 1)
                            sparse_pts_to_reproj_yellow.push_back(cv::Point3f(l_top.x, l_top.y, disparity));
                        if(l_det.class_idx == 3)
                            sparse_pts_to_reproj_big_orange.push_back(cv::Point3f(l_top.x, l_top.y, disparity));
                        else
                            sparse_pts_to_reproj_blue.push_back(cv::Point3f(l_top.x, l_top.y, disparity));
                        // start from right centers
                        sparse_pts_to_reproj_test.push_back(cv::Point3f(r_top.x, r_top.y, -disparity));
                        counter_match++;
                    }
                }
            }
        }
        std::cout << "added boxes: " << counter_match << std::endl;
        ...
        for (int i = 0; i < real_3D_pts_blue.size(); i++)
        {
            ...
            std::cout << "X: " << real_3D_pts_blue[i].z << " Y: " << real_3D_pts_blue[i].x << " Z: " << real_3D_pts_blue[i].y << std::endl;
        }
        ...
        for (int i = 0; i < real_3D_pts_yellow.size(); i++)
        {
            ...
            std::cout << "X: " << real_3D_pts_yellow[i].z << " Y: " << real_3D_pts_yellow[i].x << " Z: " << real_3D_pts_yellow[i].y << std::endl;
        }
        ...
        for (int i = 0; i < real_3D_pts_big_orange.size(); i++)
        {
            ...
            std::cout << "X: " << real_3D_pts_big_orange[i].z << " Y: " << real_3D_pts_big_orange[i].x << " Z: " << real_3D_pts_big_orange[i].y << std::endl;
        }
        ...
        // cv::projectPoints(real_3D_pts_blue, zero_tvec, zero_tvec, left_cam_mat, zero_dist_coeffs, left_img_points);
        // cv::projectPoints(real_3D_pts_blue, rvec, tvec, right_cam_mat, zero_dist_coeffs, right_img_points); 
